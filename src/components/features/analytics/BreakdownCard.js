...
export default function BreakdownCard({ title, subtitle, icon, color, items, total, limit = 6, emptyText }) {
  return (
    <div className="glass glass-hover rounded-2xl p-6">
      ...
      <p className="text-xs text-gray-400 dark:text-slate-400 mt-1 mb-4">{subtitle}</p>
      {items.length > 0 ? (
        <div className="space-y-3">
          {items.slice(0, limit).map(item => (
            <div key={item.label} className="flex items-center gap-3">
              <span className="flex-1 text-sm text-gray-700 dark:text-slate-300 truncate">{item.label}</span>
