...
export default function PageSkillsForm({ page, initialSkills }) {
  ...
  return (
    <SectionBox>
      <form onSubmit={save}>
        <h2 className="text-xl font-semibold mb-5 text-gray-900">Skills & Technologies</h2>
            ...
            return (
              <div className="flex flex-col sm:flex-row gap-2">
                <input
                  ...
                  className="flex-grow rounded-xl px-3.5 py-2.5 border border-gray-200 bg-white text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all"
                />
                {!hideProgress && (
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs font-semibold text-indigo-600 w-10 text-right">{newSkillProficiency}%</span>
                  </div>
                )}
                <button
                  type="button"
        ...
        <div className="mb-5">
          <h3 className="font-medium text-gray-700 mb-3 flex items-center gap-2 text-sm">
            <div className="w-6 h-6 bg-indigo-50 rounded-lg flex items-center justify-center">
              <FontAwesomeIcon icon={CATEGORIES.find(c => c.name === activeCategory)?.icon || faCode} className="text-indigo-500 text-xs" />
            </div>
            {activeCategory}
              ...
              return (
                <div className="flex flex-wrap gap-2">
                  {currentCategorySkills.map((skill) => (
                    <div
                      key={skill.name}
                      className="inline-flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-3.5 py-2 hover:border-indigo-200 transition-all duration-200"
                    >
            ...
            return (
              <div className="space-y-2">
                {currentCategorySkills.map((skill) => (
                  <div
                    key={skill.name}
                    className="bg-white border border-gray-200 rounded-xl p-3.5 flex flex-col sm:flex-row sm:items-center gap-3 hover:border-indigo-200 transition-all duration-200"
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-2 py-1">
                        ...
                        />
                        <span className="text-xs font-semibold text-indigo-600 w-10 text-right">{skill.proficiency}%</span>
                      </div>
        ...
        <div className="bg-gray-50/80 rounded-xl p-4 mb-5 border border-gray-100">
          <h3 className="font-medium text-gray-700 mb-3 text-sm">All Categories</h3>
            ...
            {CATEGORIES.map(cat => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[10px] text-gray-500 leading-tight">{cat.name}</div>
                </button>
              );
            })}
          </div>
